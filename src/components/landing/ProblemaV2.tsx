import { C, G, R, S, disp, bodyFont, inkBorder, card } from '@/lib/cm'
          ...
          {DOLORES.map((d, i) => (
            <Reveal key={d.titulo} delay={i * 0.1}>
              <div style={{ ...card, overflow: 'hidden', height: '100%' }}>
                <div style={{ height: 5, background: d.top }} />
                <div style={{ padding: 'clamp(24px, 3vw, 36px)' }}>
                  <h3 style={{ ...disp, fontSize: 'clamp(19px, 2.2vw, 26px)', margin: 0, marginBottom: 16 }}>
                    {d.titulo}
                  </h3>
                  <p style={{ ...bodyFont, fontSize: 'clamp(15px, 1.1vw, 18px)', lineHeight: 1.65, margin: 0, color: C.ink2 }}>
                    {d.cuerpo}
                  </p>
                </div>
              </div>
