...
export default function PreguntasAdmin() {
  ...
  const fetchPreguntas = useCallback(() => {
    ...
    if (tipoExamen) params.set('tipoExamen', tipoExamen)
  ...
  }, [page, search, especialidad, tipoExamen])
  ...
  const pctColor = (p: number) => (p >= 70 ? 'var(--success)' : p >= 50 ? 'var(--accent-dark)' : 'var(--error)')
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
        <input type="text" placeholder="Buscar por enunciado o tema..." value={search} onChange={(e) => setSearch(e.target.value)} className="flex-1 px-4 py-2.5 rounded-xl text-sm" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-primary)' }} />
        ...
        <select value={tipoExamen} onChange={(e) => { setTipoExamen(e.target.value); setPage(1) }} className="px-4 py-2.5 rounded-xl text-sm" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-primary)' }}>
          <option value="">Todos los tipos de examen</option>
          {tiposExamen.map((t) => <option key={t.id} value={t.id}>{t.nombre}</option>)}
        </select>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: '2px solid var(--border)' }}>
              {['Especialidad', 'Examen', 'Tema', 'Dificultad', 'Multimedia', 'Resp.', '% Acierto', 'Acciones'].map((h, i) => (
                <th key={h} className={`${i === 7 ? 'text-right' : 'text-left'} py-3 px-4 font-semibold ${i === 2 ? 'hidden md:table-cell' : ''}`} style={{ color: 'var(--text-muted)' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? Array.from({ length: 5 }).map((_, i) => (
              <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                {Array.from({ length: 8 }).map((_, j) => <td key={j} className="py-3 px-4"><div className="skeleton h-5 rounded w-20" /></td>)}
              </tr>
            )) : preguntas.length === 0 ? (
              <tr><td colSpan={8} className="py-12 text-center" style={{ color: 'var(--text-muted)' }}>No se encontraron preguntas</td></tr>
            ) : preguntas.map((p) => {
              ...
              return (
                <tr key={p.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td className="py-3 px-4 font-mono" style={{ color: 'var(--text-muted)' }}>{p.respondidas}</td>
                  <td className="py-3 px-4 font-bold" style={{ color: p.porcentaje === null ? 'var(--text-muted)' : pctColor(p.porcentaje) }}>{p.porcentaje === null ? '—' : `${p.porcentaje}%`}</td>
                  <td className="py-3 px-4">
